...
```bash
WORKFLOW=$(curl -L -s -H "Authorization: Bearer $PTOKEN" "https://api.up42.dev/projects/$PROJ/jobs" | jq -j '.data[0] | .workflow.id')
```
...
```bash
URL_POST_JOB="https://api.up42.dev/projects/$PROJ/workflows/$WORKFLOW/jobs"
curl -s -L -X POST -H "Authorization: Bearer $PTOKEN" -H
'Content-Type: application/json' $URL_POST_JOB -d @job_params_5a21eaff-cdaa-48ab-bedf-5454116d16ff.json
```

### Get the job information

```bash
URL_JOB_INFO=https://api.up42.dev/projects/$PROJ/jobs/$JOB"
curl -s -L -H "Authorization: Bearer $PTOKEN" $URL_JOB_INFO | jq '.' > jobs_job-$JOB.json
```

It returns a file with the following content:
